...
// ✅ FIXED: Custom Token Verification Middleware
const verifyCustomToken = async (req, res, next) => {
    try {
        ...
        // ✅ DECODE CUSTOM TOKEN MANUAL
        // Custom token kita adalah JWT, jadi bisa di-decode
        try {
            ...
            // Cari user di database berdasarkan UID
            const usersRef = db.ref('users');
            ...
            console.log('✅ User authenticated - ID:', userId, 'Email:', userData.email);
            next();
...
// ✅ FIXED: Add new dog dengan CUSTOM TOKEN auth
app.post('/api/dogs', verifyCustomToken, async (req, res) => {
    try {
...
// ✅ FIXED: Get dogs by owner dengan CUSTOM TOKEN auth
app.get('/api/my-dogs', verifyCustomToken, async (req, res) => {
    try {
